<div>
            ...
            @endcan
                <div class="card-body border-bottom py-3">
                    ...
                    <div class="ms-auto text-muted">
                        ...
                        <input type="text" class="form-control form-control-sm" aria-label="Search" wire:model='Search' placeholder="Search UACS Code">
                        </div>
                    </div>
                    </div>
                </div>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <div class="btn-group">
                                    ...
                                    <a href="" class="btn btn-sm btn-warning" wire:click.prevent="realignUACS({{$Allocation->id}})">Realign</a> &nbsp;
                                    <a href="" class="btn btn-sm btn-danger" wire:click.prevent="deleteUACS({{$Allocation->id}})">Delete</a> 
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
          <form class="modal-content" method="POST"
            ...
            <div class="modal-body">
                ...
                @if(!is_null($selectedYear)  && $rem_bal != 0)
                <div class="mb-3">
                    ...
                    <select class="form-select" wire:model="selectedUACS">
                            ...
                            @forelse ($uacs_ids as $uacs_id)
                                <option value="{{ $uacs_id->id }}">{{ $uacs_id->uacs }}</option>
                            @empty
    ...
    <div wire:ignore.self class="modal modal-blur fade" id="uacs_realign" tabindex="-1" role="dialog" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
        <div class="modal-dialog modal-dialog-centered" role="document">
          <form class="modal-content" method="POST"  wire:submit.prevent='updaterealignUACS()'>
            <div class="modal-header">
              <h5 class="modal-title">Realign UACS</h5>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                ...
                <div class="mb-3">
                    <label class="form-label">Office</label>
                    <input type="text" class="form-control" wire:model="from_office" readonly>
                    <span class="text-danger"> @error('from_office')
                        {{ $message }}
                ...
                <div class="mb-3">
                    <label class="form-label">Expense Class</label>
                    <input type="text" class="form-control" wire:model="from_expense_class" readonly>
                    <span class="text-danger"> @error('from_expense_class')
                        {{ $message }}
                ...
                <div class="mb-3">
                    <label class="form-label">P/A/P</label>
                    <input type="text" class="form-control" wire:model="from_papid" readonly>
                    <span class="text-danger"> @error('from_papid')
                        {{ $message }}
                ...
                <div class="mb-3">
                    <label class="form-label">Year</label>
                    <input type="text" class="form-control" wire:model="from_year" readonly>
                    <span class="text-danger"> @error('from_year')
                        {{ $message }}
                ...
                <div class="mb-3">
                    <label class="form-label">UACS</label>
                    <input type="text" class="form-control" wire:model="from_uacs" readonly>
                    <span class="text-danger"> @error('from_uacs')
                        {{ $message }}
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" wire:model="from_rem_bal_uacs" readonly>
                    <span class="text-danger"> @error('from_rem_bal_uacs')
                        {{ $message }}
                ...
                <div class="dropdown-divider"></div>
                @if(!is_null($selected_allocation_id)  && $from_rem_bal_uacs != 0)
                <div class="mb-3">
                    <label class="form-label">UACS </label>
                    <select class="form-select" wire:model="selectedUACS">
                            <option value="" selected>--- Choose UACS ---</option>

                            @forelse ($UACSids as $uacs_id)
                                @if ($uacs_id->uacs != $from_uacs)
                                <option value="{{ $uacs_id->id }}">{{ $uacs_id->uacs }}</option>
                                @endif
                ...
                @if(!is_null($from_rem_bal_uacs) && $from_rem_bal_uacs != 0 && !is_null($selected_allocation_id) )
                <div class="mb-3">
                    <label class="form-label">Allocation Amount</label>
                    <input type="number" class="form-control" name="amount" placeholder="Enter Allocation Amount" wire:model="realign_amount" step="0.01">
                    <span class="text-danger"> @error('realign_amount')
                        {{ $message }}
            ...
            <div class="modal-footer">
              <button type="button" class="btn me-auto" data-bs-dismiss="modal">Close</button>
              <button type="submit" class="btn btn-primary">Realign</button>
    ...
    <div wire:ignore.self class="modal modal-blur fade" id="uacs_add_confirm" tabindex="-1" role="dialog" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
