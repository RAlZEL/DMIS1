...
class Closed extends Component
{
    ...
    public function getClosedListsProperty() {
       
       
       
        $Employee = Employee:: where('email', auth('web')->user()->email )->get()->first();
       return Document::orderby('updated_at','desc')->where('is_active',false)->where('officeid', $Employee->officeid)->where('divisionid', $Employee->divisionid)->where('unitid', $Employee->unitid)->search(trim($this->Search))->paginate($this->perPage);
    ...
    public function render()
    {
        ...
        $ClosedLists = $this->ClosedLists;
        return view('livewire.user.mail.closed',[
