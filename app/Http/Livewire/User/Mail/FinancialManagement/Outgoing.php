...
public function addRoute() {
    ...
    public function updatedselectedPageRows($value) {
        ...
        if ($value) 
        {
            $this->selectedRows = $this->OutgoingLists->pluck('id')->map(function($id) {
                return (string) $id;
            });
        }
        else{
    ...
    public function getOutgoingListsProperty() {
       
       
       
        $Employee = Employee:: where('email', auth('web')->user()->email )->get()->first();
       return voucher::orderby('created_at','desc')->where('is_forwarded',true)->where('from_officeid', $Employee->officeid)->where('from_divisionid', $Employee->divisionid)->where('from_unitid', $Employee->unitid)->search(trim($this->Search))->paginate($this->perPage);
    ...
    {      
        $OutgoingLists = $this->OutgoingLists;
        return view('livewire.user.mail.financial-management.outgoing', [
